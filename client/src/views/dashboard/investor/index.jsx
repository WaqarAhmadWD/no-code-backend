...
const childRef = useRef();;
  const name = "Investement";
  const module = "investement";
  ...
  const handleDelete = async (id) => {
    const result = await dispatch(
      fetchData({ url: `/${module}/delete/${id}`, method: "DELETE" })
    ).unwrap();
    if (!result?.error) {
  ...
  const handleEdit = async (id) => {
    const result = await dispatch(
      fetchData({ url: `/${module}/edit-status/${id}`, method: "PUT", data: formData })
    ).unwrap();
    if (!result?.error) {
      fetchDataFun(false,childRef.current.getData().query);
    ...
    const result = await dispatch(
      fetchData({
        ...
        data: {
          ...
          company_id: formDataAdd.company_id.value,
        },
      })
    ).unwrap();
    if (!result?.error) {
      childRef.current.increaseField();
      fetchDataFun(false,childRef.current.getData().query);
      setFormDataAdd({
        user_id: "",
        company_id: "",
  ...
  const fetchDataFunUserCompanies = async (id) => {
    const result = await dispatch(
      fetchData({
        url: `/assign_user_company/get-users-company/${id}`,
        loading: false,
        error: false,
        message: false,
      })
          ...
          <div className="bg-white py-5  rounded-md shadow-md">
            <div className="flex flex-col md:flex-row items-center justify-between px-3">
              ...
              <div className="flex flex-col md:flex-row items-center justify-end gap-2">
                ...
                <div className="basis-1/1" onClick={() => setIsOpenAdd(true)}>
                  <button
                    type="button"
                    ...
                    <div className="space-y-4">
                      <div>
                        <Creatable
                          ...
                          onChange={(selected) => {
                            fetchDataFunUserCompanies(selected.value);
                            setFormDataAdd({
                      ...
                      <div>
                        <Creatable
                          ...
                          className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                          placeholder="Company"
                          value={formDataAdd.company_id} // Ensure this binds correctly to user_id
                          onChange={(selected) =>
                            setFormDataAdd({
                              ...formDataAdd, // Preserve existing form data
                              company_id: selected ? selected : null, // Set user_id based on selected value
                            })
                          }
                        />
                      </div>
                      <div>
                        <input
                          ...
                          value={formDataAdd.capital_contribution}
                          onChange={(e) =>
                            setFormDataAdd({
                              ...formDataAdd,
                              capital_contribution: e.target.value,
                            })
                          }
                        />
                      </div>
                      <div>
                        <input
                          ...
                          value={formDataAdd.capital_invested}
                          onChange={(e) =>
                            setFormDataAdd({
                              ...formDataAdd,
                              capital_invested: e.target.value,
                            })
                          }
                        />
                      </div>
                ...
                <table className="min-w-full table-auto rounded-lg">
                  ...
                  <thead className="">
                    <tr className=" blue-bg text-white capitalize">
                      <th className="py-2 text-xs px-6 text-center">ID</th>
                      <th className=" py-2 text-xs px-6 text-left">User</th>
                      ...
                      <th className="py-2 text-xs px-6 text-center">
                        Activity
                      </th>
                      <th className="py-2 text-xs px-6 text-center">
                        year ending captial
                      ...
                      <th className="py-2 text-xs px-6 text-center">
                        year ending captial_per
                      </th>
                      <th className="py-2 text-xs px-6 text-center">
                        investment ownership per
                      </th>
                      <th className="py-2 text-xs px-6 text-center">Status</th>
                      <th className="py-2 text-xs px-6 text-center">Year</th>
                  ...
                  <tbody className="text-gray-600 text-sm font-light  p-4">
                    {data.length > 0 ? (
                      data.map((item, index) => (
                        ...
                        >
                          <td className="flex items-center py-3 px-6 text-left text-xs text-black">
                            {item?.id}
                          </td>
                          ...
                          <td className="py-3 px-6 text-left text-xs text-black">
                            {item?.capital_contribution}
                          </td>
                          <td className="py-3 px-6 text-left text-xs text-black">
                            {item?.capital_invested}
                          </td>
                          <td className="py-3 px-6 text-left text-xs text-black">
                            {item?.activity}
                          </td>
                          <td className="py-3 px-6 text-left text-xs text-black">
                            {item?.yr_ending_captial}
                          </td>
                          <td className="py-3 px-6 text-left text-xs text-black">
                            {item?.yr_ending_captial_per}
                          </td>
                          <td className="py-3 px-6 text-left text-xs text-black">
                            {item?.investment_ownership_per}
                          </td>
                          <td className="py-3 px-6 text-left text-xs text-black">
