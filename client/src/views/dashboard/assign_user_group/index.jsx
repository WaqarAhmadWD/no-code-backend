...
UserAssignUser.propTypes = {
  permissions: PropTypes.func,
};
function UserAssignUser({permissions}) {
  const [isOpen, setIsOpen] = useState(false);
  const [formData, setFormData] = useState({ group_id: [], user_id: "" });
  const [optionsGroup, setOptionsGroup] = useState([]);
  const [data, setData] = useState([]);
  const dispatch = useDispatch()
const childRef = useRef();
  const name = "Assign Group to a User";
  const module = "assign_user_group";
  ...
  const fetchDataFun = async (notif = true,query="") => {
    ...
    if (!option?.error) {
      setOptionsGroup(
        option.data.map((item) => ({ value: item.id, label: item.name }))
      );
    }
  ...
  const handleEdit = async (id) => {
    const group_id = formData?.group_id?.map((e) => e?.value) 
    if(!id || !group_id){
      return;
    }
    const result = await dispatch(
      fetchData({ url: `/assign_user_group/create`, method: "POST",  data: {
        group_id,
        user_id: id,
      }, })
    ).unwrap();
                ...
                <table className="min-w-full table-auto">
                  ...
                  <thead className="">
                    <tr className=" blue-bg text-white capitalize">
                      ...
                      <th className="py-2 text-xs px-6 text-left">Group</th>
                      ...
                      data.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="py-3 px-6 text-left text-xs text-black">
                                               ...
                                               {item?.groups?.length <= 3 ? item?.groups?.map((group) => group?.name).join(", ") || "No Group":(item?.groups?.slice(0,3)?.map((group) => group?.name).join(", ")+"...") || "No Group"}
                                      ...
                                      <div className="mb-5 flex flex-row items-center justify-between">
                                        ...
                                        >
                                          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M12 2C17.53 2 22 6.47 22 12C22 17.53 17.53 22 12 22C6.47 22 2 17.53 2 12C2 6.47 6.47 2 12 2ZM15.59 7L12 10.59L8.41 7L7 8.41L10.59 12L7 15.59L8.41 17L12 13.41L15.59 17L17 15.59L13.41 12L17 8.41L15.59 7Z" fill="#4A90E2" />
                                          </svg>
                                        </button>
                                      </div>
                                        ...
                                        <div className="space-y-4">
                                          <div>
                                            <Select
                                              ...
                                              id="role"
                                              defaultValue={() =>{
                                                return item?.groups?.map((e)=> ({value:e.id,label:e.name}))
                                              }}
                                              isMulti
                                              ...
                                              onChange={(selected) =>
                                                setFormData({
                                                  ...
                                                  group_id: selected
                                                    ? selected
                                                    : [], // Map selected optionsGroup to an array of values
                                                })
                                              }
                                            />
                                          </div>
