...
function UserManage({ permissions }) {
...
const childRef = useRef();;
  ...
  const fetchDataFun = async (notif = true,query="") => {
    const result = await dispatch(
      fetchData({
        url: `/${module}/get`+query+`${query?"&":"?"}allowActive=true`,
        loading: true,
        error: notif,
        message: false,
        refresh: !notif
  ...
  const handleEdit = async (id) => {
    const result = await dispatch(
      fetchData({ url: `/${module}/edit/${id}`, method: "PUT", data: formData })
    ).unwrap();
    if (!result?.error) {
  ...
  const handleDelete = async (id) => {
    const result = await dispatch(
      fetchData({ url: `/${module}/delete/${id}`, method: "DELETE",refresh:"/auth/get" })
    ).unwrap();
    if (!result?.error) {
  ...
  const handleAdd = async () => {
    if (!formDataAdd?.name) {
      await dispatch(
        fetchData({
          throwMe: "Name is required",
        })
      ).unwrap();
      return;
    }
    if (!formDataAdd?.email) {
      await dispatch(
        fetchData({
          throwMe: "email is required",
        })
      ).unwrap();
      return;
    }
    if (!formDataAdd?.password) {
      await dispatch(
        fetchData({
          throwMe: "password is required",
        })
      ).unwrap();
      return;
    }
    ...
    if (!result?.error) {
      ...
      fetchDataFun(false,childRef.current.getData().query);
      setFormDataAdd({ name: "", email: "",password:"", active: true });
      setIsOpenAdd(null);
    }
  };
  return (
          ...
          <div className="bg-white py-5  rounded-md shadow-md">
            <div className="flex flex-col md:flex-row items-center justify-between px-3">
              ...
              <div className="basis-1/4">
                <h1 className="text-xl blue-text font-weight700">
                  {name} Management
                </h1>
              </div>
            ...
            <div className="grid mt-4">
              <div className="col-span-12 overflow-x-auto   rounded-lg">
                <table className="min-w-full table-auto">
                  {/* Table Header */}
                  <thead className="">
                    <tr className=" blue-bg text-white capitalize">
                      <th className="py-2 text-xs px-6 text-left">ID</th>
                      <th className=" py-2 text-xs px-6 text-left">Name</th>

                      <th className=" py-2 text-xs px-6 text-left">Email</th>
                      <th className=" py-2 text-xs px-6 text-left">Active</th>
